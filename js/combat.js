...
function renderHeroSquad() {
    ...
    playerSquad.forEach((hero, index) => {
      ...
      card.innerHTML = `<div class="portrait-frame"><img src="${hero.imageSrc}"></div>
        <div class="stats-box"><h2>${hero.name}</h2>
          <div class="bar-container"><div class="bar-fill hp-fill"></div><div class="shield-fill" style="display:none;"></div><div class="bar-text"></div></div>
          <div class="bar-container en-bar-cont" style="border-color:#ff007c"><div class="bar-fill en-fill"></div><div class="bar-text"></div></div>
  ...
  playerSquad.forEach((hero, index) => {
    ...
    card.querySelector(".hp-fill").style.width =
      `${(hero.hp / hero.baseHp) * 100}%`;
    card.querySelectorAll(".bar-text")[0].innerText = Math.floor(hero.hp);
    card.querySelector(".en-fill").style.width =
      `${(hero.energy / hero.maxEnergy) * 100}%`;
...
function renderSquad() {
    ...
    enemySquad.forEach((enemy, index) => {
      ...
      card.innerHTML = `<div class="portrait-frame"><img src="${enemy.imageSrc}"></div>
        <div class="stats-box">
          ...
          <h2>${enemy.name}</h2>
          <div class="bar-container toughness-container"><div class="bar-fill toughness-fill"></div><div class="bar-fill toughness-preview"></div></div>
          <div class="bar-container"><div class="bar-fill hp-fill"></div></div>
        </div>`;
      card.onclick = () => {
        if (enemy.hp <= 0) return; // 🚨 FIX: Prevent clicking dead enemies!
  ...
  enemySquad.forEach((enemy, index) => {
    ...
    if (enemy.hp <= 0) {
      ...
      // 🚨 FIX: Strip target classes when they die so the UI cursor disappears
